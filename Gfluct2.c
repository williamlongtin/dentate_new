...
#define mynormrand mynormrand_Gfluct2
 extern double mynormrand( double , double );
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 2, "pointer");
 	...
 	ivoc_help("help ?1 Gfluct2 C:/Users/mbezaire/Desktop/NEURON/Gfluct2.mod\n");
...
double mynormrand (  double _lmean , double _lstd ) {
   double _lmynormrand;
...
static double _hoc_mynormrand(void* _vptr) {
 double _r;
    _hoc_setdata(_vptr);
 _r =  mynormrand (  *getarg(1) , *getarg(2) );
 return(_r);
}
...
static double _nrn_current(double _v){double _current=0.;v=_v;{ {
   if ( tau_e  == 0.0 ) {
     g_e = std_e * mynormrand ( _threadargscomma_ 0.0 , 1.0 ) ;
     }
   if ( tau_i  == 0.0 ) {
     g_i = std_i * mynormrand ( _threadargscomma_ 0.0 , 1.0 ) ;
     }
   g_e = g_e0 + g_e1 ;
   if ( g_e < 0.0 ) {
     g_e = 0.0 ;
...
{
 { error =  oup();
 if(error){fprintf(stderr,"at line 174 in file Gfluct2.mod:\n	SOLVE oup\n"); nrn_complain(_p); abort_run(error);}
 }}}
